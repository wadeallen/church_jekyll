module Jekyll
  class PortfolioIndex < Page
    def initialize(site, base, dir)
      ...
      self.process(@name)
      self.read_yaml(File.join(base, '_layouts'), 'sermon-page.html')
      self.data['projects'] = self.get_projects(site)
    end

    def get_projects(site)
      {}.tap do |projects|
        Dir['_sermons/*.yml'].each do |path|
  ...
  class GeneratePortfolio < Generator
    ...
    def write_portfolio(site)
      if Dir.exists?('_sermons')
        Dir.chdir('_sermons')
        Dir["*.yml"].each do |path|
          name = File.basename(path, '.yml')
          self.write_project_index(site, "_sermons/#{path}", name)
        end
    ...
    def write_portfolio_index(site)
      portfolio = PortfolioIndex.new(site, site.source, "/sermons")
    ...
    def write_project_index(site, path, name)
      project = ProjectIndex.new(site, site.source, "/sermons/#{name}", path)
