module Jekyll
  class TeamIndex < Page
    def initialize(site, base, dir)
      ...
      self.read_yaml(File.join(base, '_layouts'), 'pastors.html')
  ...
  class GenerateTeam < Generator
    ...
    def write_team(site)
      if Dir.exists?('_pastors')
        Dir.chdir('_pastors')
        Dir["*.yml"].each do |path|
          name = File.basename(path, '.yml')
          self.write_person_index(site, "_pastors/#{path}", name)
        end
    ...
    def write_team_index(site)
      team = TeamIndex.new(site, site.source, "/pastors")
    ...
    def write_person_index(site, path, name)
      person = PersonIndex.new(site, site.source, "/pastors/#{name}", path)
      ...
      if page
        ...
        "".tap do |output|
          authors.each do |author|
            data     = YAML.load(File.read(File.join(site['source'], '_pastors', "#{author.downcase.gsub(' ', '-')}.yml")))
